...
const FONT_VERSION = '3.1.11';
const FONT_SOURCE = path.join(
  hexo.theme_dir,
  'font-source',
  `zpix-v${FONT_VERSION}.ttf`
);
const FONT_OUTPUT_DIR = 'fonts/zpix';
const COMMON_PAGE_THRESHOLD = 3;
const FONT_BLOCK_START = '<!-- zpix-font:start -->';
const FONT_BLOCK_END = '<!-- zpix-font:end -->';
const DYNAMIC_STYLE_ROUTE = 'css/style.css';
const DYNAMIC_STYLE_START = '/* zpix-dynamic:start */';
const DYNAMIC_STYLE_END = '/* zpix-dynamic:end */';
const FONT_BLOCK_PATTERN = new RegExp(
  `${FONT_BLOCK_START}[\\s\\S]*?${FONT_BLOCK_END}\\s*`,
  'g'
);
const DYNAMIC_STYLE_PATTERN = /\/\* zpix-dynamic:start \*\/[\s\S]*?\/\* zpix-dynamic:end \*\/\s*/g;
...
hexo.extend.filter.register('after_generate', async function buildZpixSubsets() {
  if (!fs.existsSync(FONT_SOURCE)) {
    throw new Error(`Zpix source not found: ${FONT_SOURCE}`);
  }
  ...
  if (!commonFont) {
    throw new Error('Zpix common subset is empty.');
  }
...
${DYNAMIC_STYLE_END}`;
  ...
  for (const page of pages) {
    ...
    const faces = [
      fontFace('Zpix Common', commonFont.url, null, 4),
      pageFont ? fontFace('Zpix Page', pageFont.url, null, 4) : ''
    ].filter(Boolean);
...
${FONT_BLOCK_END}`;
  ...
  hexo.log.info(
    'Zpix: %d pages, common %s, page subsets avg %s / max %s, %d dynamic shards',
    pages.length,
